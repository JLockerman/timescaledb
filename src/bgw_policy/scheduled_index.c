...
bgw_policy_scheduled_index_tuple_found(TupleInfo *ti, void *const data)
{
	...
	*policy = STRUCT_FROM_TUPLE(ti->tuple, ti->mctx, BgwPolicyScheduledIndex, FormData_bgw_policy_scheduled_index);
...
ts_bgw_policy_scheduled_index_insert(BgwPolicyScheduledIndex *policy)
{
	...
	Relation rel = heap_open(catalog_get_table_id(catalog, BGW_POLICY_SCHEDULED_INDEX), RowExclusiveLock);
