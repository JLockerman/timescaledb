	...
	if (!HeapTupleIsValid(idxtuple))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("could not add scheduled_index policy because the provided index is not a valid "
						"relation")));

	indexForm = (Form_pg_index) GETSTRUCT(idxtuple);
	if (indexForm->indrelid != ht->main_table_relid)
		elog(ERROR,
			 "could not add scheduled_index policy because the provided index is not a valid index on the "
			 "hypertable");
...
scheduled_index_add_policy(PG_FUNCTION_ARGS)
{
	...
	if (existing != NULL)
	{
		...
		{
			elog(WARNING,
				 "could not add scheduled_index policy due to existing policy on hypertable with different "
				 "arguments");
			return -1;
		}
	...
	Assert(DirectFunctionCall2(nameeq, NameGetDatum(index_name), NameGetDatum(&optional_index_info->fd.hypertable_index_name)));
	...
	if (policy == NULL)
	{
		...
		else
		{
			...
			if (hypertable_name != NULL)
				ereport(NOTICE,
						(errmsg("scheduled_index policy does not exist on hypertable \"%s\", skipping",
								hypertable_name)));
			else
				ereport(NOTICE,
						(errmsg("scheduled_index policy does not exist on unnamed hypertable, skipping")));
			PG_RETURN_VOID();
		}
	}
